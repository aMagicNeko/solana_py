...
def calculate_amount_out(amount, pool_info, direction):
    # Constants for liquidity fees
    LIQUIDITY_FEES_NUMERATOR = 997
    LIQUIDITY_FEES_DENOMINATOR = 1000

    # Decimals for token0 and token1
    token0_decimals = pool_info['coin_decimals']
    token1_decimals = pool_info['pc_decimals']

    # Pool amounts for token0 and token1
    pool_token0_amount = pool_info['pool_coin_amount']
    pool_token1_amount = pool_info['pool_pc_amount']

    if direction:
        reserve_in = pool_token0_amount
        reserve_out = pool_token1_amount
        amount_in_decimals = token0_decimals
        amount_out_decimals = token1_decimals
    else:
        reserve_in = pool_token1_amount
        reserve_out = pool_token0_amount
        amount_in_decimals = token1_decimals
        amount_out_decimals = token0_decimals

    # Convert the input amount to the appropriate decimals
    #amount_in = amount * 10 ** amount_in_decimals
    amount_in = amount
    # Calculate fees and effective amount in
    ...
    #amount_out = amount_out_raw / 10 ** amount_out_decimals
    amount_out = amount_out_raw
    return amount_out
def calculate_amount_in(amount, pool_info, direction):
    # Constants for liquidity fees
    LIQUIDITY_FEES_NUMERATOR = 997
    LIQUIDITY_FEES_DENOMINATOR = 1000
    ...
    # Pool amounts for token0 and token1
    pool_token0_amount = pool_info['pool_token0_amount']
    pool_token1_amount = pool_info['pool_token1_amount']
