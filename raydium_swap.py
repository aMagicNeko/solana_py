...
import wallet
from compute_price import calculate_amount_out, calculate_amount_in
...
def process_swap(pool_addr: str, step: SwapStep):
    pool_keys = fetch_pool_keys(pool_addr)
    # Check if pool keys exist
    if pool_keys is None:
        print("No pools keys found...")
        return None
    # Determine the mint based on pool keys
    base = pool_keys['base_mint']
    if step.zero_for_one:
        if base == SOL:
            buy(pool_addr, step.amount_in)
        else:
            sell(pool_addr, step.amount_in)
    else:
        if base == SOL:
            sell(pool_addr, step.amount_in)
        else:
...
def simulate_process(pool_addr: str, step: SwapStep, follow_addr: Pubkey):
    pool_keys = fetch_pool_keys(pool_addr)
    # Check if pool keys exist
    if pool_keys is None:
        print_message(f"No pools keys found...{pool_addr}", 0)
        return None
    base = pool_keys['base_mint']
    quote = pool_keys['quote_mint']
    print_message(f"{base}:{quote}", 0)
    if step.zero_for_one:
        if base == Pubkey.from_string(SOL):
            #buy
            sol_out = follow_ratio * step.amount_in
            token_in = swap_token_amount_base_in(sol_out, step.pool_coin, step.pool_pc, step.zero_for_one)
            wallet.follow_wallets[follow_addr].buy(quote, token_in, sol_out)
        else:
            #sell 
            sell_num = step.amount_in * follow_ratio
            sell_num = wallet.follow_wallets[follow_addr].compute_sell_num(base, sell_num)
            sol_in = swap_token_amount_base_in(sell_num, step.pool_coin, step.pool_pc, step.zero_for_one)
            wallet.follow_wallets[follow_addr].sell(base, sell_num, sol_in)
    else:
        if base == Pubkey.from_string(SOL):
            sell_num = step.amount_in * follow_ratio
            sell_num = wallet.follow_wallets[follow_addr].compute_sell_num(quote, sell_num)
            sol_in = swap_token_amount_base_in(sell_num, step.pool_coin, step.pool_pc, step.zero_for_one)
            wallet.follow_wallets[follow_addr].sell(quote, sell_num, sol_in)
        else:
            sol_out = follow_ratio * step.amount_in
            token_in = swap_token_amount_base_in(sol_out, step.pool_coin, step.pool_pc, step.zero_for_one)
            wallet.follow_wallets[follow_addr].buy(base, token_in, sol_out)
    print_message(f"{wallet.follow_wallets[follow_addr]}", 0)